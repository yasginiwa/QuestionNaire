...
router.post('openid', async (ctx, next) => {
    const appid = 'wx6e12c795fe0e73c5',
...
//  查询此openid用户是否有填写调查问卷
router.get('naire', async (ctx, next) => {

    const openid = ctx.query.openid

    const res = await db.query(`select * from t_naire where openid='${openid}'`)
...
//  提交一封家书
router.post('addnaire', async (ctx, next) => {

    const { openid, company, confirm, countoffset, timeoffset, attitude, box, date, channel, sugguestion, rate } = ctx.request.body

    const res = await db.query(`insert into t_naire values (default, '${openid}', '${company}', '${confirm}', '${countoffset}', '${timeoffset}', '${attitude}', '${box}', '${date}', '${channel}', '${sugguestion}', '${rate}', default)`)
